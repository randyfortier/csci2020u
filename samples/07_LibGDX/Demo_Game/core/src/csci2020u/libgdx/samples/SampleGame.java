...
public class SampleGame extends ApplicationAdapter {
	...
	// AI
	private FollowEntity sidekickEntity = null;
	private FollowEntity playerEntity = null;
	...
	private Texture sidekickTexture;
	...
	public void create() {
		...
		player = BodyFactory.createBoxBody(world, 0, 100, 70, 70, false);
		sidekick = BodyFactory.createBoxBody(world, 20, 120, 20, 20, false);
		sidekick.setGravityScale(0f);
		...
		sidekickTexture = new Texture("images/ghost.png");

		jumpSound = Gdx.audio.newSound(Gdx.files.internal("audio/grunt_sound.wav"));
		...
		// AI
		sidekickEntity = new FollowEntity(sidekick, 20);
		playerEntity = new FollowEntity(player, 70);

		Arrive<Vector2> arrive = new Arrive<Vector2>(sidekickEntity, playerEntity);
		arrive.setTimeToTarget(0.01f);
		arrive.setArrivalTolerance(2f);
		arrive.setDecelerationRadius(200f);
		sidekickEntity.setBehaviour(arrive);
	}
	...
	public void render() {
		...
		batch.draw(sidekickTexture,
				sidekick.getPosition().x * WORLD_TO_SCREEN - (sidekickTexture.getWidth() / 2),
				sidekick.getPosition().y * WORLD_TO_SCREEN - (sidekickTexture.getHeight() / 2));
